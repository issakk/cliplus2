...
pub fn add(hwnd: HWND, settings: &Settings) -> bool {
    ...
    // the shell from the system icon set because hInstance is null.
    data.icon = unsafe { win::LoadIconW(0, win::IMI_APPLICATION as usize as *const u16) };
...
fn show_menu() {
    ...
    unsafe {
        ...
        append(menu, win::MF_STRING, CMD_OPEN_SETTINGS, "打开设置文件");
...
fn handle_command(command: i32) {
    ...
    match command {
        ...
        CMD_OPEN_SETTINGS => open(APP_DIR.get().map(String::as_str)),
        CMD_AUTOSTART => {
