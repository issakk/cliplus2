...
界面统一用 Microsoft YaHei UI，字号在 96 DPI 下是 16 px，再乘显示器缩放，再乘一个**你自己的界面缩放**（`UiScale`，默认 125%，设置窗口里可改 50-300%）：进程是 per-monitor DPI aware，两个窗口都按显示器 DPI 换算，所以 4K 屏上如果系统缩放还停在 100%，字就只有 100% 那么大——**嫌小就把 `UiScale` 往上调，别去动系统的缩放**，整个界面（字、行高、窗口大小）会一起放大。100% 表示「和系统画的一样大」。
...
| `UiScale` | `125` | 界面缩放百分比，见上。50-300，100 = 跟系统一样大。改完保存就生效（弹窗下次弹出、设置窗口下次打开） |
