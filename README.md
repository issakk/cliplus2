...
6. 弹窗顶部是实例 tab：「全部」是所有机器混排，「本机」只列这台机器，其余每台机器一个 tab。在 tab 那一行的空白处（或弹窗四周的留白）按住左键可以直接拖动弹窗，**位置会被记住**（写进 `settings.json` 的 `PopupPosition`），下次打开还在那儿——不再跟着鼠标跑了。第一次打开（还没拖过）时，弹窗居中在你鼠标所在的那块屏幕上；如果记住的位置因为拔了显示器/改了分辨率跑到屏幕外，下次打开会被拉回最近那块屏幕里。
