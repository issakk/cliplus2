...
**固定**（列表里 `Ctrl+P`）不是给条目改一个字段——文件是不可变的，改了就等于重写一个已经同步出去的文件，冲突随之而来。固定状态是**旁边多一个空文件**：

```
<stem>.clip.json     内容，写一次永不改写
<stem>.pin           空的，存在即代表固定
```
...
- **别处删掉的条目，本机可能会延迟消失**。同步客户端删本地文件时文件监听见得到，条目立刻移除；但本机当时没在运行的话，就要等下次启动重建索引。故意不做墓碑文件，见 `ClipStore` 的类注释。
