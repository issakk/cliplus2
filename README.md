...
2. 按 `Win+Alt+V` 打开历史，输入即过滤，`↑↓` 选择，`Enter` 粘贴回原窗口，`Esc` 取消。打开时弹窗会主动抢到前台（否则收不到按键），**点别处失去焦点就自动收起**。
3. `Ctrl+C` 把选中的放回剪贴板但**不粘贴**（不切窗口、不注入按键），`Ctrl+P` 固定/取消固定，`Ctrl+Tab` 在实例 tab 之间切换。
4. 每条记录都带来源，弹窗第二行就是它：`文本 · 02-14 15:32 · 本机 · chrome.exe · 那个窗口当时的标题`。来源在**复制的那一刻**读，所以标题就是当时那个窗口的标题（最长存 120 字，行里放不下会省略）。搜索会一起匹配来源，输入 `chrome` 就是「Chrome 里复制过的东西」，输入标题里的词也能找到。第一行是内容预览，一行放不下全部，所以多行的记录前面标 `[N 行]`（超过内联上限的记录只留开头，标 `[多行]`，完整内容在 `.bin`，第二行末尾会写明）。时间默认不带年份，**只有不是今年的**记录才写成 `2025-02-14 15:32`（列表按时间倒序，见到年份就是旧的；搜索输入 `2025` 也能筛出这些）。
5. 多选：`Ctrl`/`Shift` 点行，或 `Shift+↑↓` 扩选，或 `Ctrl+A` 全选。多选后 `Ctrl+C` 按列表顺序**每行一条**拼成一段文本（图片在多项复制里会被跳过；只选一条时图片仍按图片复制）。
6. 弹窗顶部是实例 tab：「全部」是所有机器混排，「本机」只列这台机器，其余每台机器一个 tab。在 tab 那一行的空白处（或弹窗四周的留白）按住左键可以直接拖动弹窗，**位置会被记住**（写进 `settings.json` 的 `PopupPosition`），下次打开还在那儿——不再跟着鼠标跑了。第一次打开（还没拖过）时，弹窗居中在你鼠标所在的那块屏幕上；如果记住的位置因为拔了显示器/改了分辨率跑到屏幕外，下次打开会被拉回最近那块屏幕里。弹窗**也可以拉伸**：鼠标贴到任意一条边或角就是缩放光标（右下角那两道短杠就是这个意思），拖它改大小，松手即记住（`settings.json` 的 `PopupSize`，单位是 96 DPI 下的逻辑像素，所以换到缩放不同的显示器上会等比放大而不是原样搬过去）；最小拖到「搜索框 + tab 条 + 三行列表」，比工作区还大的尺寸下次打开会被收回来。
