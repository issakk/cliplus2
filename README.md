...
4. 多选：`Ctrl`/`Shift` 点行，或 `Shift+↑↓` 扩选，或 `Ctrl+A` 全选。多选后 `Ctrl+C` 按列表顺序**每行一条**拼成一段文本（图片在多项复制里会被跳过；只选一条时图片仍按图片复制）。
5. 弹窗顶部是实例 tab：「全部」是所有机器混排，「本机」只列这台机器，其余每台机器一个 tab。在 tab 那一行的空白处（或弹窗四周的留白）按住左键可以直接拖动弹窗——位置只对这一次显示有效，下次按热键仍然弹在鼠标旁边。
6. 托盘右键是「设置…」「打开同步目录」「打开设置文件」「退出」。设置里的热键框点一下、再按组合键就录进去了，不用手写 `Win+Alt+V`。

界面统一用 Microsoft YaHei UI，字号在 96 DPI 下是 16 px（弹窗和设置窗口同一档；12 px 的系统字号在 1080p/100% 下偏小，就是原来的观感），再乘显示器缩放：进程是 per-monitor DPI aware，两个窗口都按显示器 DPI 换算，4K 屏上如果系统缩放还停在 100%，字就只有 100% 那么大。
